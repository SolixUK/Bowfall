...
| Blood *(new)* | Every bit of damage you deal **heals you for 25%** of it | Hemorrhage (full-draw hits make the target bleed 2/s for 4s, which heals you too) · Frenzy (double healing below half health) · Transfusion (healing past full goes to your most hurt teammate within 300px) · *Blood Pact* (45% lifesteal, 15 less max health) |
...
**Players on controllers (same screen):** under the profile on the main screen, **Add player on controller** asks the new player to press any button on their controller, which becomes theirs (up to three extra players). They type a name to play as a guest, or sign in: pick a friend of the main player from the list and they only type the password. The main player (mouse and keyboard, or the first controller) runs the menus. Extra players join practice, custom games (they choose a team) and ranked searches (as part of the main player's party; ranked needs every extra player signed in, because they're rated on their own accounts). In lobbies and the draft they change element with left and right on their D-pad, role with up and down, team with LB and RB, and ready with A; X, Y and B pick their upgrades. Each has a coloured P2/P3/P4 marker and their own reticle. Online, each extra player has their own connection to the server (`/api/device-login` gives it a session without replacing the main player's).
