// Elo ratings for signed-in players, updated after every online game.
...
'use strict';
const START = 1000, K = 20, K_NEW = 32, NEW_GAMES = 20;
...
// rec: a game record (p: [{ id, b, tm, ro, w }], df: bot difficulty); users: Map of sim player id -> account
...
function rateGame(rec, users) {
  ...
  const ratingOf = (p, role) => {
    ...
    if (p.b && !u) return botElo; // AI players from matchmaking have their own account and rating
    if (!u) return START;
    const c = u.career || {};
  ...
  for (const p of rec.p) {
    ...
    const c = u.career || {};
    const k = (c.games || 0) < NEW_GAMES ? K_NEW : K;
