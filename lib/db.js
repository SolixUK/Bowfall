...
const BOARD_KEYS = ['elo', 'wins', 'kills', 'games', 'matchWins', 'ring', 'bull'];
...
function pgStore(url, pgModule) {
  ...
  const user = r => r && { id: r.id, name: r.name, passHash: r.pass_hash, admin: !!r.is_admin, title: r.title || null, ach: r.ach || {}, career: r.career || {}, country: r.country || null, created: +new Date(r.created_at) };
  return {
    kind: 'postgres',
    async init() {
    ...
    async saveUser(u) { await q('UPDATE users SET title = $2, ach = $3, career = $4, is_admin = $5, country = $6 WHERE id = $1', [u.id, u.title, JSON.stringify(u.ach || {}), JSON.stringify(u.career || {}), !!u.admin, u.country || null]); },
...
function fileStore(file) {
  ...
  return {
    ...
    async saveUser(u) { const x = d.users.find(v => v.id === u.id); if (!x) return; Object.assign(x, { title: u.title, ach: clone(u.ach || {}), career: clone(u.career || {}), admin: !!u.admin, country: u.country || null }); save(); },
