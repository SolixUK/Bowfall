...
module.exports = function createSocial(ctx) {
  ...
  const eloOfKey = key => { const ws = conns.get(key); return ws && ws.user ? eloOfUser(ws.user) : ELO_START; };
  ...
  function partyView(key) {
    ...
    return { id: p.id, leader: p.leader, you: key, mode: p.mode, queued: p.queued != null ? p.queued : null, fill: FILL_AFTER,
      members: p.members.map(k => ({ key: k, name: nameOf(k), elo: eloOfKey(k), lv: conns.get(k) && conns.get(k).user ? levelOf(conns.get(k).user.career).lv : null })) };
  }
  function pushState(key) {
    const ws = conns.get(key); if (!ws) return;
    send(ws, Object.assign({ t: 'social', you: key, account: !!ws.user, party: partyView(key) }, ws.user ? friendsOf(ws.user) : { friends: [], inReq: [], outReq: [] }));
  }
