...
module.exports = function createSocial(ctx) {
  ...
  function profileOf(key) {
    ...
    return { key, name: nameOf(key), guest: ws && !ws.user ? 1 : 0, elo: eloOfKey(key, '1v1'), eloT: eloOfKey(key, '2v2'), games: c.games || 0, wins: c.wins || 0, kills: c.kills || 0,
      ...
      el: ws && ws.mmEl, ro: ws && ws.mmRo, avatar: ws && ws.user && ws.user.ach ? ws.user.ach.avatar || null : null, own: ws && ws.user && ws.user.admin ? 1 : 0, inGame: keyInGame(key) ? 1 : 0, local: ws && ws.localOf ? 1 : 0 };
  }
  ...
  function friendsOf(u) {
    const s = soc(u);
    const list = id => { const f = ctx.live.get(id); return f ? Object.assign({ id, name: f.name, online: online(id), inGame: inGame(id), lv: levelOf(f.career).lv, elo: eloOfUser(f), cc: flagOf(f) }, online(id) ? activity(id) : {}) : null; };
  ...
  return { handle, gone, presence, initAI, useTicket, arrived, freeAI, aiEvents, aiReply, draftReady, isAI: u => !!(u && u.career && u.career.ai), MODES, FILL_AFTER, _tick: tick, _ai: () => aiUsers };
};
